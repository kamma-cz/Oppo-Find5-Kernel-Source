...
static int SN3193_power(int on)
{
	if (on) {
		...
		if (get_pcb_version() == 20) {	//evt
			if (lvs5){
				regulator_disable(lvs5);
				regulator_put(lvs5);
			}
		}else if (get_pcb_version() >= 30){ //dvt
			if (ldo121) {
			 ...
			 const char *buf, size_t count)
{
	...
	u8 t123,t4;
	//printk("huyu----%s:   buf = %s: count = %d\n", __func__, buf, count);
	//printk("huyu----%s:   totalMS = %d: onMS = %d\n", __func__, totalMS, onMS);
	SN3193_setCurrent_sled(0x01);
	...
	{
		//printk("huyu----%s:   color_R = %d: color_G = %d: color_B = %d,\n", __func__, color_R, color_G,color_B);
					...
					enum led_brightness value)
{
	...
	{
		//printk("huyu----%s: light the red ,value = %d", __func__, value);
		//SN3193_TurnOnOut_sled(RED_SLED);
	...
	{
		//printk("huyu----%s: light the blue ,value = %d", __func__, value);
		//SN3193_TurnOnOut_sled(BLUE_SLED);
